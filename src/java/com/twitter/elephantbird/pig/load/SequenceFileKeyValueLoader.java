...
/**
 * This file is intended to load the Key and Value part of a SequenceFile into a Pig schema. 
 * Use <a href="#SequenceFileValueOnlyLoader">SequenceFileValueOnlyLoader</a>  
 * if only the Value needs to be loaded into the schema.</br>
 * Files suitable for this loader would tend to be created in Map Reduce with a 'meaningful' key and 
 * carefully separated value.  The <a href="#SequenceFileStorage">SequenceFileStorage</a> will create compatible
 * files, but the key is just the rownumber and not so interesting...</br>
 * 
 * Use:</br>
 * When instantiated with no separator this load returns a single tuple with two fields to pig,
 * containing the "Key" and then the "Value".</br>
 * 
 * When instantiated with a separator, it assumes that the Value contains a number of fields
 * delimited by the separator. The separator is only valid within the Value as the Key is automatically detected.</br>
 * 
 ...
 *	);
 * </pre>
 *	The first line of input would be (1,a,b,c,d)</br>
 *	The second line of input would be (2,w,x,y,z)</br>
 *	The Key values (0 and 1) are always the first element in the schema
 *
 * @see SequenceFileValueOnlyLoader
 */

public class SequenceFileKeyValueLoader extends FileInputLoadFunc {
  ...
  protected static final Log LOG = LogFactory.getLog(SequenceFileKeyValueLoader.class);
  ...
  public SequenceFileKeyValueLoader() {
    mProtoTuple = new ArrayList<Object>(2);
  }

  public SequenceFileKeyValueLoader(String delimiter) {
